...
Sift never writes directly to Chronicle. It emits enrichment candidates via Signal files to the `signal` payload field in the journal entry. Chronicle decides promotion.

## Inter-skill interfaces

Sift writes Signal files to Chronicle (via journal signal payload): the `signal` payload field in the journal entry

Every Signal must include the `user_relevance` field (`"user"` or `"agent_only"`). Chronicle decides promotion.
