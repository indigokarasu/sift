...
- **Free web search (parallel fan-out)** — all of the following fire simultaneously:
  - **N2 MCP** (`n2_web_search`) — SearXNG-backed, 70+ engines, no API key required. Registered in the platform MCP config (runner: `npx -y n2-free-search`). Also provides `n2_news_search` for recency-focused queries.
